...
// Include global navigation menu
include 'header.php';

// Redirect user to login page if session role is missing
...
// Process form actions securely via POST requests
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    
    // Restrict building operations to super administrator role
    if ($role === 'admin') {
        
        // Handle new building creation and generate a manager account
        if ($_POST['action'] === 'add_batiment') {
            ...
            // Insert new manager profile into user table
            ...
            // Link building entry to the generated manager account
        ...
        // Handle building deletion and clean up linked dependencies
        elseif ($_POST['action'] === 'del_batiment') {
            ...
            if ($bat = mysqli_fetch_assoc($res)) {
                ...
                $message = "<div class='msg-success'> Bâtiment et son gestionnaire supprimés.</div>";
            }
        }
    }

    // Shared actions accessible by both administrators and managers
    if ($_POST['action'] === 'add_capteur') {
        $nom_cap = htmlspecialchars($_POST['nom_capteur']);
        $id_salle = (int)$_POST['id_salle'];
        
        // Add new sensor to selected room layout
    ...
    // Process equipment removal requests
    elseif ($_POST['action'] === 'del_capteur') {
        ...
        $message = "<div class='msg-success'> Capteur déconnecté et supprimé.</div>";
    }
}

// Adjust fetch scope depending on active privilege levels
if ($role === 'admin') {
...
// Fetch lists to dynamically generate selection inputs
